...
describe("exec approvals safe bins", () => {
  ...
  it("allows unprofiled safe-bin entries when explicitly opted in", () => {
    if (process.platform === "win32") {
      return;
    }
    ...
    const result = evaluateShellAllowlist({
      command: "gh pr list",
      allowlist: [],
      safeBins: normalizeSafeBins(["gh"]),
      trustedSafeBinDirs: new Set([binDir]),
  ...
  it("allows unprofiled safe-bin with flags and subcommands", () => {
    if (process.platform === "win32") {
      return;
    }
    const tmp = makeTempDir();
